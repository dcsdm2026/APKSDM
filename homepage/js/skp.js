 ...
 */
(function () {
    // Semua variabel sekarang aman di dalam scope privat SKP
    let currentPage = 1;
    ...
    async function ambilDataSkp() {
        ...
        try {
            ...
            const { data, error, count } = await query.order("tahun_skp", { ascending: false }).range(from, to);
                    ...
                    <tr class="hover:bg-slate-50 transition border-b border-slate-100">
                        ...
                        <td class="p-3 text-center">
                            <div class="flex justify-center gap-2">
                                <button onclick="skpModule.editData('${d.id}')" class="text-amber-500 hover:text-amber-700 p-1 text-xs"><i class="fa-solid fa-pen-to-square"></i></button>
                                <button onclick="skpModule.hapusData('${d.id}')" class="text-red-500 hover:text-red-700 p-1 text-xs"><i class="fa-solid fa-trash-can"></i></button>
                            </div>
                        </td>
                    </tr>
                `;
            }).join('');
        } catch (err) { console.error("Gagal memuat tabel SKP:", err.message); }
    }
    ...
    async function handleAutocomplete(val) {
        ...
        try {
            const { data, error } = await supabase.from("pegawai").select("nik, nama, nip, jabatan").ilike("nama", `%${val}%`).limit(5);
            ...
            box.innerHTML = data.map(p => `
                <div onclick="skpModule.pilihPegawai('${p.nik}', '${p.nama}', '${p.nip || ''}', '${p.jabatan || ''}')" class="p-2 text-xs hover:bg-slate-100 cursor-pointer transition flex flex-col">
    ...
    async function simpanData(e) {
        ...
        try {
            ...
            const payload = { nik, nama, nip, jabatan, pejabat_penilai, atasan_pejabat_penilai, tahun_skp, capaian_kinerja_organisasi, predikat_kinerja_pegawai, catatan_rekomendasi };
    ...
    // Hanya satu objek ini yang terekspos ke global window
    window.skpModule = {
        init: initSkpModule,
        bukaModal: bukaModal,
